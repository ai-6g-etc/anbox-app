...
stage1(){
	...
	#ubuntu 2004 test command
	#libboost-filesystem1.67.0 libboost-system1.67.0 docker.io dkms libboost-iostreams1.67.0
	...
	#ubuntu 2004 test command
	#sudo apt install -y libprocess-cpp3=3.0.1*
	#sudo apt install -y libdbus-cpp5=5.0.*
	#sudo apt install -y libdbus-cpp-dev=5.0.*
...
stage2() {
	...
	#SHOULD remove binder_linux and insert again when the arm64 server restart:
	#sudo rmmod binder_linux; sudo modprobe binder_linux num_devices=254
	#check result
	#parallels@ubuntu-linux-20-04-desktop:~/robox/kernel/robox-modules$ lsmod|grep -e ashmem_linux
	#binder_linux          126976  0
	#parallels@ubuntu-linux-20-04-desktop:~/robox/kernel/robox-modules$ lsmod|grep -e binder_linux
	...
	echo "build robox"
	#for 2004: echo y | sudo apt install python2
	...
	pushd sources
	if [ ! -d mesa ]; then
		#from https://stackoverflow.com/questions/21181231/server-certificate-verification-failed-cafile-etc-ssl-certs-ca-certificates-c/69403278#69403278
	...
	#for 2004 and build-dep not work: echo y | sudo apt install bison flex libxcb-dri3-dev libx11-xcb-dev libxcb-glx0-dev libxcb-dri2-0-dev libxcb-xfixes0-dev libxcb-present-dev libxcb-sync-dev libxshmfence-dev libvdpau-dev libsensors4-dev
...
setup_android_image() {
	...
	elif [ -f images/android_zj_1222.img ]; then
		echo "Another Android image exist, skip download"
		ANDROID_IMG=android_zj_1222.img
	else
		mkdir -p images
	...
	#sha256:b994f131a938bb9f3134856cdddcc79ab02bad6466ffea660ed472b70dd34922
	#parallels@ubuntu-linux-20-04-desktop:~/Android_Container/android$ sudo docker images
	#REPOSITORY   TAG                  IMAGE ID       CREATED         SIZE
	#android      robox_with_exagear   b994f131a938   5 seconds ago   860MB
}
